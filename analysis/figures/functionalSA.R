...
t_annotationsRBP_figure <- t_annotationsRBP_annotated %>%
  ...
  mutate(fat = case_when(idgene < 47 ~ 1,
                         47 <= idgene & idgene < 94 ~ 2,
                         94 <= idgene & idgene < 141 ~ 3,
                         idgene >= 141  ~ 4))  %>%
...
fig_SystematicAnalysis <- t_annotationsRBP_figure %>%
  ...
  theme(aspect.ratio = 5,
        ...
        panel.border = element_rect(color="black", fill = NA), legend.title = element_text(size=8),
