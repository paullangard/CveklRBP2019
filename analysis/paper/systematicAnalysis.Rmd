...
t_proteomicZhao2018_RBP <- t_proteomicZhao2018 %>% 
  #filter for RNA-binding proteins
  filter_all(any_vars(str_detect(., pattern = "RNA-binding"))) %>%
  #exclude wrongly annotated RBP (manual exclusion)
  filter(!GeneNames %in% c('Tsn','Impdh2','Nudt5','Tdrkh','Trir','Jmjd6','Ro60', "Trove2")) %>%
  filter(rankEpith < 1800 | rankFibers < 1800) %>%
  mutate(tissue = case_when(
    #define gene as epi, fiber or both if it passed the threshold in one or two tissues
    (rankEpith < 1800 & rankFibers >= 1800) ~ "epi",
    (rankEpith >= 1800 & rankFibers < 1800) ~ "fibers",
    (rankEpith < 1800 & rankFibers < 1800) ~ "both"
  )) %>% 
  dplyr::select(gene=GeneNames, starts_with("rank"), tissue) %T>%
  write_tsv("../data/derived_data/proteomicZhao2018_RBP.tsv")
```
...
###########################################################
t_DAVID_annotationsRBP <- read_tsv("../data/raw_data/DAVID_annotationsRBP.tsv") %>%
  #select all annotations columns
  dplyr::select(ID, UP_KEYWORDS, starts_with("GO"), starts_with("KEGG"), BIOCARTA) %>%
  #gather all columns in one
  unite(col = "KEYWORDS", UP_KEYWORDS, BIOCARTA, KEGG_PATHWAY, starts_with("GOTERM"), sep="") %>%
...
t_CVEKL_annotationsRBP <- read_tsv("../data/raw_data/CVEKL_annotationsRBP.tsv",
  col_types = "cccc",
...
#compile annotations
###########################################################
t_functionalGridRBP <- read_tsv("../data/raw_data/RBP_functionalannotationsGrid.tsv")
...
```{r include=T, echo=FALSE, warning=F, message=F, fig.cap="\\textbf{\\label{fig:articlevennRBP}Systematic analysis of lens-expressed RBPs.} (A) Venn diagram of highly-expressed RBPs grouped in either lens epithelial or fiber cells or both. (B) Functional representativity of lens-expressed RBPs, divided with regard of target RNAs class. (C) Dotplot showing the functional classification of expressed RBPs in the lens. The tissue specificity of an RBP is indicated by a colored sign next to its name.", fig.height=9, fig.width=7}
...
#dev.off()
```
```{r include=T, echo=FALSE, warning=F, message=F, fig.width=6.29, fig.height=2.9} 
t_ISYTE_enrichment <- read_tsv("../data/raw_data/ISYTE_enrichment.tsv") %>% dplyr::rename(gene=Symbol)
...
t_proteomicEnrichment %>%
  ...
  geom_point() + 
  geom_point(data=filter(t_proteomicEnrichment_interest, tissue=="both"), mapping = aes(x=rankEpith, y=rankFibers, size=enrich_category), color="#c0c0c0", shape=16, inherit.aes = F, show.legend = F) +
    geom_point(data=filter(t_proteomicEnrichment_interest, tissue=="epi"), mapping = aes(x=rankEpith, y=rankFibers, size=enrich_category), color="#0000ff", shape=17, inherit.aes = F, show.legend = F) +
  geom_point(data=filter(t_proteomicEnrichment_interest, tissue=="fibers"), mapping = aes(x=rankEpith, y=rankFibers, size=enrich_category), color="#ff0000", shape=15, inherit.aes = F, show.legend = F) +
 ...
 geom_text_repel(data=t_proteomicEnrichment_interest, 
                 ...
                 inherit.aes = F,# segment.alpha = .5,
                 fontface=2, 
                 color="black", segment.color = "black",
                 size=2.4, box.padding =  unit(.2, "lines"), segment.size = .4, min.segment.length = unit(1, "mm"),
                 show.legend = F, max.iter = 10000, seed=22, force = 3) + 
                 #segment.alpha = .4,  
                 #direction = "both", 
                 #seed=4) +
  ...
  facet_wrap(~groups, scales = "free") + theme(plot.margin = margin(.05,.12,.05,.05,"cm"))
```
